...
import {
  generateWindDirectionBadge,
  windDirection,
} from '@/lib/data/sensor-data/weather/wind-direction';
...
const WindDirectionCard: FC<WindDirectionCardProps> = ({
  value,
  isLoading,
  className,
}) => {
      ...
      <CardContent className="mt-5 flex flex-col gap-4">
        <div className="flex flex-col lg:flex-row justify-between gap-4">
          <div className="flex items-start gap-2 flex-wrap">
            <img src={windDirectionIcon} alt="App Logo" className="w-6 mt-0" />
          ...
          {user && (
            <div className="flex justify-center">
              <ExportSensorDialog
                name={windDirection.title}
                sensorTypeCode={windDirection.sensorTypeCode}
                sensorType="weather"
              />
            </div>
          )}
        ...
        <div className="flex flex-col lg:flex-row gap-2">
          <div className="lg:basis-1/2">
            <div className="flex flex-col justify-center items-center gap-2 h-full">
              ...
              <div>
                {generateWindDirectionBadge(isLoading ? undefined : value)}
              </div>
            </div>
          </div>
