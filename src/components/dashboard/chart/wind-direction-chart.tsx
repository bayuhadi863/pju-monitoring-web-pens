...
const WindDirectionChart: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await getSensorChartData(windDirection.sensorTypeCode, 2);
  ...
  const chartConfig = {
    averageValue: {
      label: `${windDirection.title} Rata-rata`,
              ...
              formatter={(value, name) => (
                <div className='flex min-w-[130px] items-center text-xs font-medium gap-2'>
                  ...
                  <div className='ml-auto flex items-baseline gap-0.5 font-mono font-medium tabular-nums text-foreground'>
                    {value}
                    <span className='font-medium text-muted-foreground'>{windDirection.unit}</span>
                  </div>
                </div>
              )}
            />
