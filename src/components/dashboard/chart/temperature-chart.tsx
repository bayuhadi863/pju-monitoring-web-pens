...
const TemperatureChart: React.FC = () => {
    ...
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const response = await getSensorChartData(temperature.sensorTypeCode, 2);
                            ...
                            formatter={(value, name) => (
                                <div className='flex min-w-[130px] items-center text-xs font-medium gap-2'>
                                    ...
                                    <div className='ml-auto flex items-baseline gap-0.5 font-mono font-medium tabular-nums text-foreground'>
                                        {value}
                                        <span className='font-medium text-muted-foreground'>{temperature.unit}</span>
                                    </div>
                                </div>
                            )}
                        />
    ...
    const loadingComponent = <div className='h-64 flex justify-center items-center'>Loading...</div>;
    const errorComponent = <div className='h-64 flex justify-center items-center'>{error}</div>;
