...
const PjuControlCardNew: React.FC<PjuControlCardNewProps> = ({ pjuId, className }) => {
    ...
    return (
        <Card className={`overflow-hidden shadow-md ${className}`}>
            <CardHeader>
                ...
                <CardDescription className='text-muted-foreground'>Nyalakan, matikan, atau aktifkan mode otomatis pada lampu PJU.</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {sendLoading && timer > 0 && <p className='mt-4 text-sm text-muted-foreground'>Menunggu {timer} detik untuk bisa mengirim data lagi...</p>}
            </CardContent>
            <CardFooter>
                {isSuccess && (
